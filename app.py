...
# ========== 导入蛋白质-配体分析模块 ==========
try:
    from protein_ligand_streamlit import render_protein_ligand_tab
    PROTEIN_LIGAND_AVAILABLE = True
    st.sidebar.success("✅ 蛋白质-配体分析模块就绪")
    logging.info("蛋白质-配体分析模块导入成功")
except ImportError as e:
    PROTEIN_LIGAND_AVAILABLE = False
    st.sidebar.warning(f"⚠️ 蛋白质-配体分析模块导入失败: {e}")
    logging.warning(f"蛋白质-配体分析模块导入失败: {e}")
...
with tab4:
    ...
    if PROTEIN_LIGAND_AVAILABLE:
        render_protein_ligand_tab()
    else:
        st.error("蛋白质-配体分析模块不可用")
        st.markdown("""
        ### 安装所需依赖

        ```bash
        pip install plip py3Dmol stmol requests
        ```

        **注意事项**:
        1. PLIP可能需要额外的依赖，建议使用conda安装:
           ```bash
           conda install -c conda-forge plip
           ```
        2. 确保有可用的PDB文件或网络连接以下载PDB结构

        **示例PDB文件**:
        - [EGFR激酶 (3POZ)](https://files.rcsb.org/download/3poz.pdb)
        - [HIV蛋白酶 (1AAQ)](https://files.rcsb.org/download/1aaq.pdb)
        """)
